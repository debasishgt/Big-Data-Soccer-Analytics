...
def main(search_term, num_tweets):
    ...
    fspos = open('scores.pos','w')
    fsneg = open('scores.neg','w')
    ftpos = open('times.pos','w')
    ftneg = open('times.neg','w')
   
    db2 = client.twitter_db
    db_tweets = db2.tweets
    c_data = db.data.find({"text": {"$exists": True}})
    ii = 0
    for status in c_data:
        ...
        ii = ii + 1
        pop = sent_tweet.sentiment.polarity
        tweet['score'] = pop
        if pop > 0:
            fspos.write(str(pop))
            fspos.write('\n')
            conv = time.strptime((tweet['time']), "%a %b %d %H:%M:%S +0000 %Y")
            temptime = time.strftime("%Y-%m-%d %H:%M:%S +0000 ")
            #conv=time.strptime(from_date,"%a %b %d %H:%M:%S +0000 %Y")
            #conv1=time.strftime("%Y-%m-%d %H:%M:%S +0000 ", conv)
            ftpos.write(temptime)
            ftpos.write('\n')
   ...
   # Store data in MongoDB
    ...
    #print_results()
    fspos.close() # you can omit in most cases as the destructor will call if
    fsneg.close() # you can omit in most cases as the destructor will call if
    ftpos.close() # you can omit in most cases as the destructor will call if
    ftneg.close() # you can omit in most cases as the destructor will call if
    return
...
def search(search_term, num_tweets, auth):
    ...
    for status in c_data:
        #status = c_data.next()
                ...
                # Configure the fields you are interested in from the status object
        tempp        = status['text'].encode('utf-8')
        tempp = re.sub(r"(?:\@|https?\://)\S+", "", new_tweet)
